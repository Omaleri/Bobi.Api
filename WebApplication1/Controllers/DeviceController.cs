...
namespace Bobi.Api.Controller
{
    public class DeviceController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetListByFilterAsync(Expression<Func<Device, bool>> exp)
        {
            var result = await _deviceAppService.GetListByFilterAsync(exp);
            if (result.IsSuccess)
            {
                return Ok(result.Data);
            }
