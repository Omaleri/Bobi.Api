...
namespace Bobi.Api.Controller
{
    public class NumberController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetListByFilterAsync(Expression<Func<Number, bool>> exp)
        {
            var result = await _numberAppService.GetListByFilterAsync(exp);
            if (result.IsSuccess)
            {
                return Ok(result.Data);
            }
