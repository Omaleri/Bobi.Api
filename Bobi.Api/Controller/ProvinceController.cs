...
namespace Bobi.Api.Controller
{
    public class ProvinceController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetListByFilterAsync(Expression<Func<Province, bool>> exp)
        {
            var result = await _provinceAppService.GetListByFilterAsync(exp);
            if (result.IsSuccess)
            {
                return Ok(result.Data);
            }
